...
chrome.webNavigation.onBeforeNavigate.addListener(function(data) {
  ...
  chrome.storage.sync.get({
    ...
    if(data.frameId==0 && data.url!='http://localhost:8000/' && items.chosenblock==true)
    {
      console.log('onBeforeNavigate', data.tabId, data.url);
      updateTabBlocking(data.tabId,data.url)
...
function fetchCertInfo(hostname, callback) {
  ...
  xhr.onreadystatechange = function() {
    ...
    if (typeof this.responseText === 'undefined' || this.responseText.length === 0) {
      chrome.extension.getBackgroundPage().console.log('Error in response json');
      callback(null);
      return;
    }
  ...
  chrome.storage.sync.get({
    ...
    method=items.chosenmethod
    xhr.open('GET', 'http://localhost:8000/' + hostname, true);
    xhr.setRequestHeader('x-validate-host', hostname);
    xhr.setRequestHeader('crl-or-ocsp', method);
    xhr.send();
  });
